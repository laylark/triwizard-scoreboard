...
- `script.js` — client state machine. Keeps a local `state` object (`adminToken`, `isAdmin`, `isLoading`, `isGameEnded`, `scores`) and re-renders on every mutation via `renderScores()` → `renderTeamOrder()` + `renderLeader()` + `renderWinnerScreen()`. Every API call toggles `isLoading` around the fetch and re-renders to disable buttons.

### Key invariants

- **Admin auth is enforced on every mutation route.** `promptForAdminAccess` POSTs to `/api/admin/login`, which compares the plaintext password against `process.env.ADMIN_PASSWORD`. On match, the server responds with `{ success: true, token }` where `token` is `SHA-256(ADMIN_PASSWORD)` hex. The `requireAdmin` middleware in `server.js` is attached to every `/api/scores/rounds*`, `/api/scores/reset`, `/api/scores/end`, and `/api/scores/resume` route and rejects missing/invalid tokens with 401 using `crypto.timingSafeEqual`. `GET /api/scores` stays public. Any new mutation route must also attach `requireAdmin`.
- **Admin session is persisted in `localStorage`** under the `triwizardAdmin` key via `readPersistedAdminToken` / `writePersistedAdminToken` — the stored value is the bearer token string itself (not a boolean). The client sends it as `Authorization: Bearer <token>` via the `adminFetch` helper. On any 401 from a mutation, `handleAuthFailure` auto-logs the client out.
...
`GET /api/scores` (public), `POST /api/scores/rounds` (`{ placements }`), `DELETE /api/scores/rounds/:roundId`, `POST /api/scores/reset`, `POST /api/scores/end`, `POST /api/scores/resume`. All write endpoints require `Authorization: Bearer <token>` and return the full normalized game state — the client replaces its state wholesale via `applyGameState`, so new server-side fields flow through automatically if added to `normalizeGameState`. `POST /api/admin/login` (`{ password }`) is separate: it validates against `process.env.ADMIN_PASSWORD` and returns `{ success: true, token }` or 401 — it does not touch game state.
